...
def solve_jsp(jsp_instance: np.ndarray, plot_results: bool = True):
    ...
    while not done:
        ...
        valid_tasks = [i for i, bol in enumerate(possible_actions, start=1) if bol]
        machine_available = {
        ...
        if candidates:
            processing_times = [(i, env.G.nodes[i]["duration"]) for i in candidates]
            action, _ = min(processing_times, key=lambda data: data[1])
