...
def solve_jsp(jsp_instance: np.ndarray, plot_results: bool = True):
    ...
    while not done:
        ...
        if candidates:
            remaining_processing_times = [
                (i, nx.shortest_path_length(env.G, i, env.sink_task, weight='weight'))
                for i in candidates
            ]
            action, _ = max(remaining_processing_times, key=lambda data: data[1])
    ...
    info["LRPT_solving_duration"] = solving_duration
    info["LRPT_makespan"] = solving_duration
