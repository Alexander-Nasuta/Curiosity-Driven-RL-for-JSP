...
class Curiosity3(Exploration):
    ...
    def _postprocess_torch(self, policy, sample_batch):
        ...
        print(f"{self.model._curiosity_inverse_fcnet}")
        dist_inputs = self.model._curiosity_inverse_fcnet(phi_cat_next_phi)
        #print(f"{dist_inputs=}")
        action_dist = (
        ...
        # Calculate the ICM loss.
        loss = (1.0 - self.beta) * inverse_loss + self.beta * forward_loss
        print(f"loss: {loss.item():.4f}, forward_loss: {forward_loss.item():.4f}, inverse_loss: {inverse_loss.item():.4f}")
        ...
        print(f"#### policy global_timestep####")
        print(dir(policy))
        print(policy.global_timestep)
        print(f"#### sample_batch ####")
        print(dir(sample_batch))
        #print(sample_batch.time_major)
        #print(sample_batch.timeslices)
        print(sample_batch[SampleBatch.OBS].shape)
        print(sample_batch[SampleBatch.OBS][-1])
        print(sample_batch[SampleBatch.ACTIONS].shape)
        print(sample_batch[SampleBatch.ACTIONS][0])
