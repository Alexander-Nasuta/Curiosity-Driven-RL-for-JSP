...
def run_dynamic_ppo_icm_jss_experiment_6x6(total_timesteps: int, n_jobs: int, n_machines: int, *,
                                       eval_instance_name: str,
                                       pre_eval_learning_timesteps: int = 500_000,
    ...
    icm_config = {
        "IntrinsicCuriosityModuleWrapper": {
            ...
            "maximum_sample_size": int(12852 * 0.875),
    ...
    log.info(pprint.pformat(config["eval_env_kwargs"]))
    venv = make_vec_env_without_monitor(
        env_id=config["env_name"],
    ...
    for i in range(episode_len):
        ...
        # log.info(f"{i+1}/{episode_len}: {action}")
        obs, _, _, infos = venv.step(action)

    #log.info(pprint.pformat(infos))
